        ...
        if (newState && newState.allowed && (!oldState || !oldState.allowed)) {
          ...
          if (jobs.length > 0) {
            ...
            for (const job of jobs) {
              try {
                const resetResp = await fetch(`${base}/crawl/jobs/${job.jobId}/reset`, { method: 'POST' });
                if (resetResp.ok) {
...
async function pollOnce(cfg) {
  ...
  for (const j of jobs) {
    try {
      ...
      if (!isDomainAllowed(cfg, domain)) {
        await registerPendingDomain(domain, j.jobId, j.run_id);
        console.log('[gg-sw] domain not allowed yet, skipping job', { jobId: j && j.jobId, domain });
        await logActivity('DOMAIN', `Job ${j.jobId} blocked - domain "${domain}" needs approval`);
        await logToServer(j.run_id, 'WARN', 'Job skipped - domain awaiting user approval', { jobId: j.jobId, domain, url: j.url });
        continue;
      }
      ...
      // REMOVED: Local done jobs cache - server is source of truth
      // Smart de-duplication: check server status if job was seen recently
      const seen = await getSeenJobs();
      const sj = seen[j.jobId];
      if (sj && (Date.now() - sj.ts < 3 * 60 * 1000)) {
        console.log('[gg-sw] job seen recently, verifying server status', { jobId: j.jobId, seenAgo: Math.floor((Date.now() - sj.ts) / 1000) });
        await logToServer(j.run_id, 'DEBUG', 'Job seen recently, verifying status', { jobId: j.jobId, seenAgoSec: Math.floor((Date.now() - sj.ts) / 1000) });
        try {
          const controller = new AbortController();
          const timeoutId = setTimeout(() => controller.abort(), 5000);
          const statusResp = await fetch(`${base}/crawl/jobs/${j.jobId}/status`, { 
            method: 'GET',
            signal: controller.signal
          });
          clearTimeout(timeoutId);
          if (statusResp.ok) {
            ...
            console.log('[gg-sw] server job status', { jobId: j.jobId, status: statusData.status, attempts: statusData.attempts });
            await logToServer(j.run_id, 'DEBUG', 'Server job status verified', { jobId: j.jobId, status: statusData.status, attempts: statusData.attempts });
            if (statusData.status === 'DONE' || statusData.status === 'FAILED') {
              console.log('[gg-sw] server confirms job complete, skipping');
              await logToServer(j.run_id, 'INFO', 'Job confirmed complete on server', { jobId: j.jobId, status: statusData.status });
              delete seen[j.jobId];
            ...
            if (statusData.status === 'PENDING') {
              // Job is PENDING - needs processing, clear seen cache and proceed
              console.log('[gg-sw] server says PENDING, clearing seen cache and will process', { jobId: j.jobId });
              await logActivity('INFO', `Job ${j.jobId} is PENDING on server, clearing seen cache and processing`);
              delete seen[j.jobId];
              await setSeenJobs(seen);
              // Fall through to claim and process the job
            } else if (statusData.status === 'CLAIMED') {
              // Job is CLAIMED - either by us (still processing) or another worker
              // Skip for now, will check again in next poll
              const seenAgoSec = Math.floor((Date.now() - sj.ts) / 1000);
              console.log('[gg-sw] skip this poll, job CLAIMED by worker', { jobId: j.jobId, seenAgoSec });
              await logActivity('INFO', `Job ${j.jobId} is CLAIMED (being processed), seen ${seenAgoSec}s ago - skipping`);
              continue;
            } else {
              // Job is in some other state (DONE, FAILED, etc.)
              console.log('[gg-sw] skip this poll, job in terminal state', { jobId: j.jobId, status: statusData.status });
              await logActivity('INFO', `Job ${j.jobId} has status "${statusData.status}", skipping`);
              continue;
            }
          }
        } catch (e) {
          console.log('[gg-sw] status check failed, will process job anyway', { err: String(e) });
          await logActivity('WARN', `Job ${j.jobId} status check failed, proceeding anyway`, { error: String(e) });
          // Don't skip - proceed to process the job
        }
      }
      ...
      // Brief backoff to prevent tight loops, but allow retry sooner (2 min window)
      try {
        const seenErr = await getSeenJobs();
        seenErr[j.jobId] = { ts: Date.now(), status: 'error' };
        await setSeenJobs(seenErr);
      } catch (_) {}
      // Attempt to reset job on server for faster retry
      try {
        await fetch(`${base}/crawl/jobs/${j.jobId}/reset`, { method: 'POST' });
      } catch (_) {}
      try {
...
async function handleJob(cfg, job) {
  ...
  // Re-validate domain gating as a safety net
  const domain = getDomainFromUrl(targetUrl);
  if (!isDomainAllowed(cfg, domain)) {
    await registerPendingDomain(domain, job.jobId, job.run_id);
    console.log('[gg-sw] handleJob: domain now disallowed, aborting', { jobId: job && job.jobId, domain });
    await logActivity('ERROR', `Job ${job.jobId} domain "${domain}" no longer allowed`, { url: targetUrl });
    await logToServer(job.run_id, 'WARN', 'Job domain not allowed', { jobId: job.jobId, domain, url: targetUrl });
    return;
  }
  ...
  // Check if this is a PDF before proceeding with normal HTML flow
  await setLiveStatus({ mode: 'server', jobId: job && job.jobId, url: targetUrl, domain, phase: 'check_pdf' });
  const pdfCheck = await checkIfPdf(tabId);
  
  if (pdfCheck.isPdf) {
    console.log('[gg-sw] PDF detected, switching to PDF download flow', { jobId: job.jobId, method: pdfCheck.method });
    await logToServer(job.run_id, 'INFO', 'PDF detected, downloading for server upload', { jobId: job.jobId, url: targetUrl, detectionMethod: pdfCheck.method });
    await logActivity('INFO', `Job ${job.jobId} is a PDF document`, { url: targetUrl.substring(0, 100), method: pdfCheck.method });
    
    // Download PDF as blob
    await setLiveStatus({ mode: 'server', jobId: job && job.jobId, url: targetUrl, domain, phase: 'download_pdf' });
    try {
      const pdfResp = await fetch(targetUrl);
      if (!pdfResp.ok) {
      ...
      console.log('[gg-sw] PDF downloaded successfully', { jobId: job.jobId, sizeBytes: pdfBytes });
      await logToServer(job.run_id, 'INFO', 'PDF downloaded successfully', { jobId: job.jobId, sizeBytes: pdfBytes });
      await logActivity('SUCCESS', `Job ${job.jobId} - PDF downloaded (${Math.round(pdfBytes/1024)}KB)`);
      
      // Upload PDF to server
      await setLiveStatus({ mode: 'server', jobId: job && job.jobId, url: targetUrl, domain, phase: 'submit_pdf' });
      const t2 = Date.now();
      await postResultPdf(cfg, job.jobId, pdfBlob, job.run_id, targetUrl);
      const submitMs = Date.now() - t2;
      
      console.log('[gg-sw] PDF result posted', { jobId: job.jobId, elapsed_ms: submitMs });
      await logToServer(job.run_id, 'INFO', 'PDF submitted to server', { jobId: job.jobId, elapsed_ms: submitMs, pdfBytes });
      ...
      await logActivity('SUCCESS', `PDF Job ${job.jobId} COMPLETE - Total time: ${Math.round((Date.now() - t0)/1000)}s`, { 
        domain, 
        sizeKB: Math.round(pdfBytes/1024),
        submitMs 
      });
      ...
      if (cfg.autoCloseTab) {
        try { 
          ...
          await logActivity('INFO', `Tab ${tabId} auto-closed for PDF job ${job.jobId}`);
          await logToServer(job.run_id, 'INFO', 'Tab auto-closed', { jobId: job.jobId, tabId });
        } catch (e) {}
      }
      
      return; // Exit handleJob - PDF flow complete
    } catch (e) {
      console.log('[gg-sw] PDF download/upload error', { jobId: job.jobId, error: String(e) });
      await logToServer(job.run_id, 'ERROR', 'PDF processing failed', { jobId: job.jobId, error: String(e) });
      await logActivity('ERROR', `PDF Job ${job.jobId} failed`, { error: String(e) });
      throw e;
    }
  }
  
  // Not a PDF - continue with normal HTML extraction flow
  console.log('[gg-sw] Not a PDF, proceeding with HTML extraction', { jobId: job.jobId });
  await logToServer(job.run_id, 'INFO', 'Not a PDF, proceeding with HTML extraction', { jobId: job.jobId });
  ...
  hist.status = 'ok';
  hist.size = (html && html.length) || 0;
  // Don't store HTML for server jobs - it's already on the server in articles/ directory
  // hist.html = html;
  await pushHistory(hist);
  await clearLiveStatus();
  await appendCrawlHistory({ url: targetUrl, jobId: job.jobId, domain, ts: Date.now() });
      ...
      try {
        if (attempt === 1) {
          try {
            ...
            console.log('[gg-sw] foreground injected');
            await logActivity('INFO', `Foreground script injected into tab ${tabId}`);
          } catch (e) {
            console.log('[gg-sw] inject foreground error', String(e));
            await logActivity('ERROR', `Failed to inject foreground into tab ${tabId}`, { error: String(e) });
          }
        }
        chrome.tabs.sendMessage(tabId, { type: 'GG_GET_HTML' }, async (resp) => {
          ...
          if (chrome.runtime.lastError) {
            ...
            console.log('[gg-sw] sendMessage error', msg);
            // Retry once after injection if receiver missing
            if (attempt === 0 && /Receiving end does not exist/i.test(msg)) {
              await logActivity('WARN', `Tab ${tabId} content script not ready, retrying with injection`);
              trySend(1);
              return;
            }
            await logActivity('ERROR', `Failed to communicate with tab ${tabId}`, { error: msg });
            settled = true;
            clearTimeout(timer);
            resolve('');
            return;
...
async function postResultPdf(cfg, jobId, pdfBlob, runId, url) {
  const base = (cfg.serverBaseUrl || '').replace(/\/$/, '');
  const uploadUrl = `${base}/crawl/result/pdf`;
  const pdfBytes = pdfBlob.size;
  
  console.log('[gg-sw] postResultPdf begin', { jobId, uploadUrl, pdfBytes });
  await logActivity('INFO', `Submitting PDF for job ${jobId}`, { sizeBytes: pdfBytes });
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 60000); // 60s for larger PDFs
      ...
      if (resp.ok) {
        console.log('[gg-sw] postResultPdf ok', { jobId, status: resp.status });
        await logActivity('SUCCESS', `PDF job ${jobId} submitted successfully`, { sizeBytes: pdfBytes });
        return;
      }
      
      const text = await resp.text().catch(() => '');
      lastErr = `HTTP ${resp.status} ${text.slice(0, 300)}`;
      console.log('[gg-sw] postResultPdf http error', { jobId, status: resp.status, attempt, body: text.slice(0, 500) });
      await logActivity('ERROR', `PDF job ${jobId} submit failed - HTTP ${resp.status}`, { attempt, error: text.slice(0, 200) });
    } catch (e) {
      lastErr = String(e && e.message ? e.message : e);
      console.log('[gg-sw] postResultPdf fetch error', { jobId, attempt, err: lastErr });
      await logActivity('ERROR', `PDF job ${jobId} submit network error`, { attempt, error: lastErr });
    }
    
    if (attempt < maxAttempts) {
      const delayMs = Math.min(5000, 500 * attempt);
      console.log('[gg-sw] postResultPdf retrying', { jobId, attempt, delayMs });
      await new Promise(r => setTimeout(r, delayMs));
    }
  }
  
  const err = new Error(`postResultPdf failed after ${maxAttempts} attempts: ${lastErr}`);
  console.log('[gg-sw] postResultPdf failed permanently', { jobId, error: String(err) });
  await logActivity('ERROR', `PDF job ${jobId} permanently failed after ${maxAttempts} attempts`, { error: lastErr });
  throw err;
}
